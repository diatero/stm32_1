...
//pBuffer:数据指针
//NumToRead:字(4位)数
extern void STMFLASH_Read(unsigned int ReadAddr,unsigned int *pBuffer,unsigned int NumToRead);
...
extern void STMFLASH_Write(unsigned int WriteAddr,unsigned int *pBuffer,unsigned int NumToWrite);
...
int ReadFlashPointInformation(long long address);

//采点完成时需要调用，将所采点信息填入flash里;返回 1 代表成功读取
int PoseSamplingDone(long long address);
