...
extern int count,count2,count3,count1,count4;
extern int period[4];
extern int period_re[4];
extern int board;
extern int led;
extern int mode[4];
extern int mode_re[4];
extern  int step;
extern  int status;
extern int counti;
...
void CAN1_RX0_IRQHandler(void)//1号板用于接收按键切换。
{
	...
	if(CAN_MessagePending(CAN1,CAN_FIFO0)!=0)
	{
		...
		CAN1Buffer[i]=RxMessage.Data[i];
		led=CAN1Buffer[0];
			mode_re[led-1]=CAN1Buffer[1];
			period_re[led-1] =CAN1Buffer[2];
			status=3;
	}
...
void CAN2_RX0_IRQHandler(void)
{
	...
	if(CAN_MessagePending(CAN2,CAN_FIFO0)!=0)
	{
		...
		led=CAN2Buffer[0];
		mode_re[led -1]=CAN2Buffer[1];
		period_re[led -1] =CAN2Buffer[2];
	}
...
//		USART_SendData(USART3,data);
		 ...
		 if(len==0&&(res=='B'||res=='S'))
			 data[0]=res ;
		 else if(data[0]!=' '&&data[0]!='S'&&data[0]!='B')
			 data[0]='F';
			 if(data[0]=='B'||data[0]=='S')
			 {	
					data[len++]=res ;
					if(data[0]=='B')
						step++;
...
//			 if(data[0]=='S'&&data[1]=='t'&&data[2]=='a'&&data[3]=='t'&&data[4]=='u'&&data[5]=='s')
//				 step=1;
			 if(len==8&&step==0&&status!=1)
			 {
				 status =2;
				 len=0;
			 }
